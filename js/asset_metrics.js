...
const ASSET_METRIC_INFO = {
    ...
    factorMnav: {
        ...
        interacts: 'Sets the direction the other two factors then modify — but below 0.7x it progressively OVERRIDES them. A price crash drags all three factors down at once, yet an underwater treasury and recent underperformance are largely consequences of that same fall, so counting them fully would punish one event three times. Below 0.7x the other two are pulled toward neutral; by 0.3x the discount speaks almost alone. Note the asymmetry too: a discount is good for a buyer today, but bad for the company’s ability to issue shares accretively.',
    ...
    factorTreasury: {
        ...
        interacts: 'Most informative combined with mNAV, and deliberately DAMPED when mNAV falls below 0.7x. Being underwater is largely a consequence of the price fall that created the discount, so at a deep discount this factor is pulled toward neutral rather than compounding the same bad news. Above 0.7x it counts in full: underwater at a premium means the market is pricing something other than the coins.',
    ...
    factorRelPerf: {
        ...
        interacts: 'A fixed 3-month window is used for both companies so the factor stays comparable across tabs — Strive’s treasury era is too short for a year. Like the treasury factor it is damped below 0.7x mNAV, since recent underperformance is largely what produced the discount. Outperformance driven by an expanding premium can unwind; outperformance driven by rising BTC per share is durable.',
